...
Entity* entity_new()
{
	...
	for (i = 0;i < _entity_manager.entity_max;i++)
	{
		...
		_entity_manager.entity_list[i].rotation = 0;
...
void entity_free(Entity* self)
{
	...
	//anything else we allocate for out entity would get cleaned up here
}
...
void entity_draw(Entity* self)
{
	...
	{
		center = gfc_vector2d(
			(float)self->sprite->frame_w * 0.5f,
			(float)self->sprite->frame_h * 0.5f
		);
		gf2d_sprite_render(
			self->sprite,
			position,
