...
void world_tile_layer_build(World* world)
{
	...
	if (!world->tileSet)return;//can't draw with no tiles
	...
	for (j = 0; j < world->tileHeight;j++)
	{
		for (i = 0; i < world->tileWidth;i++)
		{
			...
			position.x = (float)(i * world->tileSet->frame_w);
			position.y = (float)(j * world->tileSet->frame_h);
	...
	{
		slog("failed to convert world tile layer texture");
		return;
	}
}
...
World* world_load(const char* filename)
{
	...
	h = sj_array_count(vertical);
	horizontal = sj_array_get_nth(vertical, 0);
	w = sj_array_count(horizontal);
	world = world_new(w, h);
	if (!world)
	{
...
void world_setup_camera(World* world)
{
	...
	{
		slog("no tiles set for world");
		return;
	}

	bounds.x = 0;
	bounds.y = 0;
	bounds.w = (float)world->tileLayer->surface->w;
	bounds.h = (float)world->tileLayer->surface->h;

	camera_set_bounds(bounds);
	camera_enable_bindings(1);
	camera_apply_bounds();
}
/*eol@eof*/